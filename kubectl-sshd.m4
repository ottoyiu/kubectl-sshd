...
# ARG_OPTIONAL_SINGLE([container], c, [Container name (if more than one container in Pod)], [])
...
KUBECTL_CMD_CONT=$KUBECTL_CMD
if [ ! -z "$CONTAINER_NAME" ];
then
    KUBECTL_CMD_CONT="${KUBECTL_CMD} -c ${CONTAINER_NAME}"
fi

copy_binary() {
    $KUBECTL_CMD_CONT exec $POD_NAME ls $PAYLOAD_PATH > /dev/null 2>&1
    if [ $? -ne 0 ]
    then
        echo "Payload binary does not exist at $PAYLOAD_PATH. Uploading..."
        $KUBECTL_CMD_CONT exec $POD_NAME -- mkdir -p $PAYLOAD_DIR
        if [ $? -ne 0 ]
        then
            echo "Payload DIR $PAYLOAD_DIR could not be created. Aborting."
            exit 1
        fi
        $KUBECTL_CMD_CONT cp $SOURCE_PATH $POD_NAME:$PAYLOAD_PATH
        if [ $? -eq 0 ]
        then
            ...
            then
                ...
                $KUBECTL_CMD_CONT exec $POD_NAME -- ln -s $PAYLOAD_PATH $REMOTE_SCP_PATH
                if [ $? -ne 0 ]
                then
...
start_ssh_server() {
    ...
    $KUBECTL_CMD_CONT exec $POD_NAME -- mkdir -p /etc/dropbear
    $KUBECTL_CMD_CONT exec -it $POD_NAME -- $PAYLOAD_PATH -RFEs -p $PORT
    set +e
}
...
cleanup() {
    ...
    $KUBECTL_CMD_CONT exec $POD_NAME -- rm $AUTHORIZED_KEYS_DEST
    $KUBECTL_CMD_CONT exec $POD_NAME -- rm -rf /etc/dropbear
    $KUBECTL_CMD_CONT exec $POD_NAME -- rm $PAYLOAD_PATH
    $KUBECTL_CMD_CONT exec $POD_NAME -- rm $REMOTE_SCP_PATH
    set +e
}
